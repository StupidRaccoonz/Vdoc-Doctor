...
import { regular, bold, doctor_withdrawal_history, api_url, withd_wal, withd_debit, doctor_withdrawal_request } from '../config/Constants';
...
const Withdrawal = () =>{
	...
	const get_withdrawal_list = async() => {
		...
		await axios({
			method: 'post', 
			url: api_url + doctor_withdrawal_history,
			data:{ id:global.id }
		})
		.then(async response => {
			setLoading(false);
	...
	const request_amt = async(val) => {
		...
		await axios({
			method: 'post', 
			url: api_url + doctor_withdrawal_request,
			data:{ doctor_id:global.id, amount:val }
		})
		.then(async response => {
			setLoading(false);
		  ...
		  <View style={{ width:'100%', flexDirection:'row', justifyContent:'center', alignItems:'center', borderWidth:0.5, padding:10, borderRadius:10, borderColor:colors.theme_fg_three, backgroundColor:colors.theme_fg_three,}}>
			...
			<View style={{ width:'60%', justifyContent:'center', alignItems:'flex-start'}}>
				...
				<View style={{ flexDirection:'row' }} >
					<Text style={{ fontFamily:regular, fontSize:11, color:colors.grey}}>{Moment(item.created_at).fromNow()}</Text>   
				</View> 
			</View>
			...
			{withdraw_list.length > 0 ?
				<ScrollView>
					...
					<View style={{ width:'100%', height:'100%', alignItems:'center', justifyContent:'center'}}>
						<Text style={{ fontFamily:bold, color:colors.theme_fg_two, fontSize:16}}>Sorry no request done yet</Text>
					</View>
				}
